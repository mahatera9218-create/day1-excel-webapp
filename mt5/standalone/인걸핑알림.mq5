//+------------------------------------------------------------------+
//|                                            인걸핑알림.mq5         |
//|   인걸핑 발생 알림 + 리테스트 추적 + engulf_log.csv               |
//|   (한 파일 완결판, #include 없음)                                 |
//|                                                                  |
//|  H1 인걸핑 감지 → 팝업 + 모바일 푸시 + 품질점수 + CSV 기록         |
//|  리테스트: 확정 후 M15 4개 안에 존 재진입 확인 → 진입/손절/익절    |
//+------------------------------------------------------------------+
...
input double InpW_Ratio      = 0;   // 품질① 삼킴비율 가중치
input double InpW_Candle     = 0;   // 품질② 캔들방향강도
input double InpW_Level      = 0;   // 품질③ 레벨근접
input double InpW_HTF        = 0;   // 품질④ 상위(H4)일치
input double InpW_Flow       = 0;   // 품질⑤ 체결일치  (0=모두 균등)
input bool   InpAlertPopup   = true;   // 팝업 Alert
input bool   InpAlertPush    = true;   // 모바일 푸시(SendNotification)
input bool   InpWriteCsv     = true;   // engulf_log.csv 기록
...
{
   return(SessionOfMinute(MinuteOfDay(ToKST(TimeCurrent(), InpServerToKST))));
}
double CurrentPrice()
{
   MqlTick t;
...
bool DetectEngulf(const ENUM_TIMEFRAMES tf, const int checkShift, EngulfResult &r, const int maxLegs)
{
   ...
   for(int leg=1; leg<=maxLegs; leg++)
   {
      ...
      bool bodyEngulf=(curTop>=pTop && curBot<=pBot);
      if(!bodyEngulf) continue;
...
struct QualityScore
{
   double engulfRatio, candleStrength, levelProx, htfAlign, flowAlign, rsi, score;
};
...
void CheckEngulf()
{
   ...
   double w[5]; w[0]=InpW_Ratio; w[1]=InpW_Candle; w[2]=InpW_Level; w[3]=InpW_HTF; w[4]=InpW_Flow;
   ...
   string msg=StringFormat(
      ...
      (htfSame>0?"O":"X"), (flowSame>0?"O":(ts.valid?"X":"-")), posPct);
...
void TrackRetest()
{
   ...
   if(PriceInZone(price,g_rtEng,InpWickZone))
   {
      ...
      string msg=StringFormat(
         ...
         g_rtBars+1, InpRetestBars, g_rtId,
